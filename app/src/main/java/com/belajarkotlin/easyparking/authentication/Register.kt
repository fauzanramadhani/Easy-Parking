...
fun Register(navHostController: NavHostController) {
    ...
    val registerButtonState = rememberSaveable {
        mutableStateOf(true)
    }
    ...
    Column(
        ...
        CButton(
            ...
            modifier = Modifier
                .fillMaxWidth()
                .padding(horizontal = 30.dp),
            enabledState = registerButtonState.value,
        ) {
            focusManager.clearFocus()
            registerButtonState.value = false
            when {
                (emailState.text.isEmpty()) -> {
                    Toast.makeText(context, "Silahkan masukan email anda", Toast.LENGTH_SHORT)
                        .show()
                    Handler().postDelayed({
                        registerButtonState.value = true
                    }, 2000)
                }
                ...
                (passwordConfirmState.text.isEmpty()) -> {
                    Toast.makeText(context, "Silahkan konfirmasi password anda", Toast.LENGTH_SHORT)
                        .show()
                    Handler().postDelayed({
                        registerButtonState.value = true
                    }, 2000)
